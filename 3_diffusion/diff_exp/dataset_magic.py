...
def process_func(path: str, aug_rate=1, missing_ratio=0.1):
    ...
    # gt_mask: 0 for missing elements and manually masked elements
...
class tabular_dataset(Dataset):
    ...
    def __init__(
        self, eval_length=10, use_index_list=None, aug_rate=1, missing_ratio=0.2, seed=0
    ):
        self.eval_length = eval_length
        np.random.seed(seed)

        dataset_path = "../../dataset/magic/magic04.data"
        processed_data_path = (
            f"./data_magic/missing_ratio-{missing_ratio}_seed-{seed}.pk"
        )
        processed_data_path_norm = (
            f"./data_magic/missing_ratio-{missing_ratio}_seed-{seed}_max-min_norm.pk"
        )
